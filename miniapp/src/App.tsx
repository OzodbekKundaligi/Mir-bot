...
type TabKey =
	...
	| 'ads'
	| 'admin'
type ContentType = 'movie' | 'serial'
type SearchType = 'all' | 'movie' | 'serial'
...
interface BootPayload {
	...
	payment: { code: string; links: string[] }
	notice: { text: string; link: string; updated_at: string }
	sections: {
		recent_movies: ContentItem[]
		recent_serials: ContentItem[]
		top_viewed: ContentItem[]
		favorites: ContentItem[]
	}
	ads: { can_create: boolean; mine: AdItem[] }
	links: { mini_app: string; bot_username: string; bot_url: string }
	admin?: {
		total_users: number
		...
		pending_ads: Array<AdItem & { channel_ref?: string }>
		ad_channels: Array<{ id: string; title: string; channel_ref: string }>
...
async function api<T>(
	...
	const payload = (await response
		.json()
		.catch(() => ({ ok: false, detail: 'Javob formati xato' }))) as T & {
		ok?: boolean
		detail?: string
	}
...
function buildBotActionLink(action: string, username?: string) {
...
function openTelegramTarget(url: string) {
	if (!url) return
...
function Media({ item }: { item: ContentItem }) {
	...
	return (
		<div className='grid h-full w-full place-items-center bg-gradient-to-br from-accent/15 to-accent-purple/20 text-sm'>
			{item.code || 'MEDIA'}
		</div>
	)
}
...
function Card({
	...
	onFav: (item: ContentItem) => void
	onReact: (item: ContentItem, reaction: 'like' | 'dislike') => void
}) {
	return (
		<article className='overflow-hidden rounded-[20px] border border-white/10 bg-white/[0.03] backdrop-blur-[40px]'>
			<button
			...
			<div className='space-y-2 p-3'>
				...
				<div className='flex gap-1.5'>
					<button
						className={cls('icon-pill', item.is_favorite && 'icon-pill-active')}
						onClick={() => onFav(item)}
					>
						<Heart
							size={13}
					...
					<button
						className={cls(
							'icon-pill',
							item.user_reaction === 'like' && 'icon-pill-active',
						)}
						onClick={() => onReact(item, 'like')}
					>
						<ThumbsUp size={13} />
					</button>
					<button
						className={cls(
							'icon-pill',
							item.user_reaction === 'dislike' && 'icon-pill-active',
						)}
						onClick={() => onReact(item, 'dislike')}
					>
						<ThumbsDown size={13} />
					</button>
					<button
						className='ml-auto rounded-full bg-accent px-3 py-1 text-[11px]'
						onClick={() => onOpen(item)}
					>
						Ochish
					</button>
...
export default function App() {
	...
	const [adChannelMap, setAdChannelMap] = useState<Record<string, string>>({})
	...
	useEffect(() => {
		const t = window.setInterval(() => {
			if (document.visibilityState === 'visible') void loadBoot(true)
		}, 30000)
	...
	useEffect(() => {
		...
		if (!adminQuery.trim()) setAdminUsers(boot?.admin?.recent_users || [])
	}, [
	...
	useEffect(() => {
		...
		setAdChannelMap(current => {
			...
			for (const ad of boot?.admin?.pending_ads || [])
				if (!next[ad.id]) next[ad.id] = first
			return next
		})
	...
	const primary = useMemo(
		() =>
			navItems.filter(n =>
				['home', 'search', 'saved', 'profile'].includes(n.key),
			),
		[navItems],
	)
	const secondary = useMemo(
		() =>
			navItems.filter(
				n => !['home', 'search', 'saved', 'profile'].includes(n.key),
			),
		[navItems],
	)
	...
	const featuredItems = useMemo(() => {
		...
		return pool
			.filter(item => {
				...
				if (seen.has(k) || !item.id) return false
				seen.add(k)
				return true
			})
			.slice(0, 8)
	...
	useEffect(() => {
		if (featuredItems.length < 2 || tab !== 'home') return
		const t = window.setInterval(
			() => setFeaturedIndex(c => (c + 1) % featuredItems.length),
			5200,
		)
		return () => window.clearInterval(t)
	}, [featuredItems, tab])
	...
	const sendToBot = useCallback(
		(payload: Record<string, unknown>) => {
			...
			if (deepLink) return openTelegramTarget(deepLink)
			if (tg?.sendData) {
	...
	const openInBot = useCallback(
		(item: ContentItem) => {
			if (item.deep_link) return openTelegramTarget(item.deep_link)
	...
	const openDetail = useCallback(
		async (item: ContentItem) => {
			await withBusy(async () => {
				const payload = await api<{ ok: true; item: ContentItem }>(
					`/api/content/${item.content_type}/${item.id}`,
				)
				setDetail(payload.item)
			}).catch((err: unknown) =>
				notify(err instanceof Error ? err.message : 'Kontent ochilmadi'),
			)
		},
		[notify, withBusy],
	)

	const refreshDetail = useCallback(async (item: ContentItem) => {
		try {
			const payload = await api<{ ok: true; item: ContentItem }>(
				`/api/content/${item.content_type}/${item.id}`,
			)
	...
	const toggleFavorite = useCallback(
		async (item: ContentItem) => {
			await withBusy(async () => {
				const payload = await api<{ ok: true; active: boolean }>(
				...
				notify(err instanceof Error ? err.message : "Saqlab bo'lmadi"),
			)
		},
		[loadBoot, notify, refreshDetail, withBusy],
	)

	const reactItem = useCallback(
		async (item: ContentItem, reaction: 'like' | 'dislike') => {
			await withBusy(async () => {
				...
				notify(err instanceof Error ? err.message : 'Reaksiya saqlanmadi'),
			)
		},
		[loadBoot, notify, refreshDetail, withBusy],
	)
	...
	const runSearch = useCallback(async () => {
		...
		try {
			const payload = await api<{ ok: true; items: ContentItem[] }>(
				`/api/search?q=${encodeURIComponent(searchQuery.trim())}&type=${encodeURIComponent(searchType)}`,
			)
	...
	const toggleNotification = useCallback(
		async (key: 'new_content' | 'pro_updates' | 'ads_updates') => {
			await withBusy(async () => {
				...
				notify(err instanceof Error ? err.message : 'Sozlama saqlanmadi'),
			)
		},
	...
	const uploadImage = useCallback(
		async (event: React.ChangeEvent<HTMLInputElement>) => {
			...
			try {
				...
				const payload = await api<{ ok: true; url: string }>(
					'/api/upload-image',
					{
						method: 'POST',
						body: { dataUrl },
	...
	const submitAd = useCallback(
		async (event: React.FormEvent) => {
			...
			await withBusy(async () => {
				...
				notify(err instanceof Error ? err.message : "E'lon yuborilmadi"),
			)
		},
	...
	const setContentMode = useCallback(
		async (mode: 'private' | 'public') => {
			await withBusy(async () => {
				...
				notify(err instanceof Error ? err.message : 'Media rejimi saqlanmadi'),
			)
		},
		[loadBoot, notify, withBusy],
	)
	...
	const saveNotice = useCallback(
		async (clear = false) => {
			await withBusy(async () => {
				...
				notify(err instanceof Error ? err.message : 'Xabar saqlanmadi'),
			)
		},
	...
	const saveProSettings = useCallback(async () => {
		await withBusy(async () => {
			...
			notify(err instanceof Error ? err.message : 'PRO sozlamalari saqlanmadi'),
		)
	...
	const searchAdminUsers = useCallback(async () => {
		...
		try {
			const payload = await api<{ ok: true; items: UserSummary[] }>(
				`/api/admin/users/search?q=${encodeURIComponent(adminQuery.trim())}`,
			)
	...
	const setUserPro = useCallback(
		async (userId: number, enabled: boolean) => {
			await withBusy(async () => {
				const payload = await api<{ ok: true; item: UserSummary }>(
					'/api/admin/users/pro',
					{ method: 'POST', body: { userId, enabled } },
				)
				...
				notify(err instanceof Error ? err.message : 'PRO holati yangilanmadi'),
			)
		},
		[loadBoot, notify, withBusy],
	)
	...
	const setUserAdmin = useCallback(
		async (userId: number, enabled: boolean) => {
			await withBusy(async () => {
				...
				notify(
					err instanceof Error ? err.message : 'Admin holati yangilanmadi',
				),
			)
		},
		[loadBoot, notify, withBusy],
	...
	const createChannel = useCallback(async () => {
		await withBusy(async () => {
			...
			notify(err instanceof Error ? err.message : "Kanal qo'shilmadi"),
		)
	...
	const deleteChannel = useCallback(
		async (channelId: string) => {
			await withBusy(async () => {
				...
				notify(err instanceof Error ? err.message : "Kanal o'chirilmadi"),
			)
		},
		[loadBoot, notify, withBusy],
	)
	...
	const reviewPayment = useCallback(
		async (requestId: string, action: 'approve' | 'reject') => {
			await withBusy(async () => {
				...
				notify(err instanceof Error ? err.message : "To'lov ko'rib chiqilmadi"),
			)
		},
		[loadBoot, notify, withBusy],
	)

	const reviewAd = useCallback(
		async (adId: string, action: 'approve' | 'reject') => {
			await withBusy(async () => {
				...
				notify(err instanceof Error ? err.message : "E'lon ko'rib chiqilmadi"),
			)
		},
	...
	const featured =
		featuredItems[featuredIndex % Math.max(featuredItems.length, 1)] || null
	const mixedLatest = [
		...boot.sections.recent_movies,
		...boot.sections.recent_serials,
	].slice(0, 8)
			...
			? [
					{
						...
						news_type: 'notice' as const,
						link: boot.notice.link,
					},
				]
			: []),
		...boot.sections.recent_movies.map(item => ({
			...item,
			news_type: 'movie' as const,
		})),
		...boot.sections.recent_serials.map(item => ({
			...item,
			news_type: 'serial' as const,
		})),
	].slice(0, 10)
	...
	return (
		<div className='mesh-bg min-h-screen text-white'>
			<div className='mx-auto max-w-[470px] px-4 pb-36 pt-4'>
				...
				{tab === 'home' ? (
					<main className='space-y-4'>
						<section className='glass-panel overflow-hidden rounded-[24px]'>
							...
							>
								{!featured?.preview_url ? (
									<div className='absolute inset-0 bg-gradient-to-br from-accent-purple/35 to-accent/20' />
								) : null}
								<div className='relative flex min-h-[280px] flex-col justify-end p-4'>
									...
									<div className='mt-3 flex gap-2'>
										<button
											...
											onClick={() =>
												featured ? void openDetail(featured) : setTab('search')
											}
										>
										...
										>
											<Heart
												size={14}
												fill={featured?.is_favorite ? 'currentColor' : 'none'}
											/>
										</button>
										<button
											className='btn-soft'
											onClick={() =>
												featured ? openInBot(featured) : undefined
											}
										>
											<ArrowRight size={14} />
										</button>
						...
						<section>
							...
							<div className='grid grid-cols-2 gap-3'>
								{boot.sections.top_viewed.map(item => (
									<Card
							...
							<div className='mt-2 flex gap-2'>
								{SEARCH_TYPES.map(item => (
									<button
										...
										className={cls(
											'chip',
											searchType === item.key && 'chip-active',
										)}
				...
				{tab === 'saved' ? (
					<main>
						{boot.sections.favorites.length ? (
							<div className='grid grid-cols-2 gap-3'>
								{boot.sections.favorites.map(item => (
									<Card
										key={`${item.content_type}-${item.id}`}
										item={item}
										onOpen={openDetail}
					...
					<main className='space-y-3'>
						<div className='glass-panel rounded-xl p-4'>
							...
							<div className='mt-2 flex gap-2'>
								<button
									className='btn-soft'
									onClick={() => copyText(String(boot.user.id))}
								>
									<Copy size={14} /> ID
								</button>
								<button
									className='btn-soft'
									onClick={() => sendToBot({ action: 'open_notifications' })}
								>
									<Bell size={14} /> Bot
								</button>
							</div>
						...
						<div className='glass-panel rounded-xl p-4 space-y-2'>
							{(
								...
								>
									...
									<button
										className={cls('chip', row.value && 'chip-active')}
								...
								<div className='mt-2 flex gap-2'>
									{item.news_type === 'notice' ? (
										...
										<>
											<button
												className='btn-primary'
												onClick={() => void openDetail(item as ContentItem)}
											>
												Ko'rish
											</button>
											<button
												className='btn-soft'
												onClick={() => openInBot(item as ContentItem)}
											>
												Botda
											</button>
										</>
					...
					<main className='space-y-3'>
						<div className='glass-panel rounded-xl p-4'>
							...
							<div className='mt-2 flex gap-2'>
								<button
									className='btn-primary'
									onClick={() => sendToBot({ action: 'open_pro' })}
								>
									Davom etish
								</button>
								<button
									className='btn-soft'
									onClick={() => copyText(boot.payment.code)}
								>
									<Copy size={14} />
								</button>
							</div>
						...
						<div className='glass-panel rounded-xl p-4'>
							...
							<div className='mt-2 flex gap-2'>
								{boot.payment.links.map((link, index) => (
									<button
										key={link}
										className='btn-soft'
				...
				{tab === 'ads' ? (
					<main className='space-y-3'>
						<div className='glass-panel rounded-xl p-4'>
							{boot.ads.mine.length ? (
								<div className='space-y-2'>
									{boot.ads.mine.map(item => (
										<div
											key={item.id}
								...
								<p className='text-sm text-white/70'>E'lonlar yo'q</p>
							)}
						</div>
						<form
							className='glass-panel rounded-xl p-4 space-y-2'
							onSubmit={event => void submitAd(event)}
						>
							{boot.ads.can_create ? (
								<>
									<input
										...
										value={adForm.title}
										onChange={event =>
											setAdForm(c => ({ ...c, title: event.target.value }))
										}
									...
									<input
										...
										onChange={event =>
											setAdForm(c => ({ ...c, buttonText: event.target.value }))
										}
										placeholder='Tugma matni'
									/>
									<input
										className='input'
										value={adForm.buttonUrl}
										onChange={event =>
											setAdForm(c => ({ ...c, buttonUrl: event.target.value }))
										}
										placeholder='https://...'
									/>
									<input
					...
					<main className='space-y-3'>
							...
							<div className='mt-2 grid grid-cols-2 gap-2'>
								{ADMIN_ACTIONS.map(item => (
									<button
										...
										onClick={() => sendToBot({ action: item.action })}
									>
										{item.label}
									</button>
								))}
						...
						<div className='glass-panel rounded-xl p-4'>
							<div className='flex gap-2'>
								<button
									className='btn-soft'
									onClick={() => void setContentMode('private')}
								>
									Yopiq
								</button>
								<button
									className='btn-soft'
									onClick={() => void setContentMode('public')}
								>
									Ochiq
								</button>
							</div>
						...
						<div className='glass-panel rounded-xl p-4'>
							...
							<div className='mt-2 space-y-2'>
								{adminUsers.length ? (
									adminUsers.map(item => (
										...
										>
											...
											<div className='mt-2 flex gap-2'>
												<button
													className='chip'
													onClick={() => void setUserPro(item.id, !item.is_pro)}
												>
												...
												<button
													...
													disabled={
														item.is_seed_admin ||
														(item.id === boot.user.id && item.is_admin)
													}
													onClick={() =>
														void setUserAdmin(item.id, !item.is_admin)
													}
												>
									...
									<p className='text-sm text-white/65'>Foydalanuvchi yo'q</p>
								)}
							</div>
						</div>
						<div className='glass-panel rounded-xl p-4'>
							...
							<textarea
								...
								onChange={event =>
									setNoticeForm(c => ({ ...c, text: event.target.value }))
								}
							...
							<input
								...
								onChange={event =>
									setNoticeForm(c => ({ ...c, link: event.target.value }))
								}
								placeholder='Havola'
							/>
							<div className='mt-2 flex gap-2'>
						...
						<div className='glass-panel rounded-xl p-4'>
							...
							<input
								className='input mt-2'
								value={proForm.priceText}
								onChange={event =>
									setProForm(c => ({ ...c, priceText: event.target.value }))
								}
								placeholder='Narx matni'
							/>
							<input
								className='input mt-2'
								type='number'
								...
								onChange={event =>
									setProForm(c => ({ ...c, durationDays: event.target.value }))
								}
						...
						<div className='glass-panel rounded-xl p-4'>
							...
							<div className='mt-2 space-y-2'>
								{(boot.admin?.ad_channels || []).map(channel => (
									<div
										key={channel.id}
										className='flex items-center justify-between rounded-lg border border-white/10 bg-white/[0.02] px-3 py-2'
									>
						...
						<div className='glass-panel rounded-xl p-4'>
							...
							<div className='mt-2 space-y-2'>
								{(boot.admin?.pending_payments || []).length ? (
									boot.admin?.pending_payments.map(item => (
										<div
											key={item.id}
											className='rounded-lg border border-white/10 bg-white/[0.02] p-3'
										>
											...
											<div className='mt-2 flex gap-2'>
												<button
													...
													onClick={() => void reviewPayment(item.id, 'approve')}
												>
													Tasdiqlash
												</button>
												<button
													className='chip chip-reject'
													onClick={() => void reviewPayment(item.id, 'reject')}
												>
													Rad etish
												</button>
											</div>
									...
									<p className='text-sm text-white/65'>To'lov yo'q</p>
								)}
							</div>
						</div>
						<div className='glass-panel rounded-xl p-4'>
							...
							<div className='mt-2 space-y-2'>
								{(boot.admin?.pending_ads || []).length ? (
									boot.admin?.pending_ads.map(item => (
										<div
											key={item.id}
											className='rounded-lg border border-white/10 bg-white/[0.02] p-3'
										>
											...
											<div className='mt-2 flex gap-2'>
												<button
													className='chip chip-approve'
													onClick={() => void reviewAd(item.id, 'approve')}
												>
													Tasdiqlash
												</button>
												<button
													className='chip chip-reject'
													onClick={() => void reviewAd(item.id, 'reject')}
												>
													Rad etish
												</button>
											</div>
									...
									<p className='text-sm text-white/65'>E'lon yo'q</p>
								)}
							</div>
						</div>
					</main>
					...
					{detail ? (
						...
						>
							<motion.div
								...
								className='fixed bottom-0 left-1/2 h-[88vh] w-full max-w-[470px] -translate-x-1/2 overflow-hidden rounded-t-[28px] border border-white/10 bg-[#121724]/95'
								onClick={event => event.stopPropagation()}
							>
								<button
									className='icon-pill absolute right-4 top-4 z-10'
									onClick={() => setDetail(null)}
								>
									<X size={14} />
								</button>
								...
								<div className='space-y-3 p-4'>
									...
									<div className='flex gap-2'>
										...
										<button
											className='btn-soft'
											onClick={() =>
												copyText(detail.deep_link || detail.code || detail.title)
											}
										>
									...
									<div className='flex gap-2'>
										<button
											className={cls(
												'btn-soft flex-1',
												detail.user_reaction === 'like' && 'btn-soft-active',
											)}
											onClick={() => void reactItem(detail, 'like')}
										>
										...
										<button
											...
											onClick={() => void reactItem(detail, 'dislike')}
										>
					...
					<div className='flex items-center justify-between gap-1'>
						{primary.map(item => {
							...
							return (
								<button
									key={item.key}
									className={cls('dock-item', tab === item.key && 'dock-item-active')}
						...
						<button
							className={cls(
								'dock-item',
								(menuOpen || secondaryActive) && 'dock-item-active',
							)}
				...
				{menuOpen ? (
					...
					>
						<motion.div
							...
							exit={{ y: 40, opacity: 0 }}
							className='absolute bottom-24 left-1/2 w-[calc(100%-28px)] max-w-[430px] -translate-x-1/2 rounded-[24px] border border-white/10 bg-[#141a28]/95 p-3'
							onClick={event => event.stopPropagation()}
						>
			...
			<AnimatePresence>
				{busy ? (
					<motion.div
						...
						animate={{ opacity: 1 }}
						exit={{ opacity: 0 }}
						className='fixed inset-0 z-[100] grid place-items-center bg-black/20 backdrop-blur-sm'
					>
